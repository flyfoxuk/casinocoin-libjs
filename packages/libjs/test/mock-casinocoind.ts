...
const account = addresses.TEST_ACCOUNT
...
module.exports = function createMockCasinocoind(port) {
  ...
  mock.on('request_account_info', function(request, conn) {
    ...
    if (request.account === account.ACCOUNT) {
      conn.send(createResponse(request, fixtures.account_info.normalTestAccount))
    } else if (request.account === addresses.ACCOUNT) {
  ...
  mock.on('request_ledger', function(request, conn) {
    ...
    if (request.ledger_index === 20) {
      conn.send(createLedgerResponse(request, fixtures.ledger.ledger_20))
    } else if (request.ledger_index === 34) {
    ...
    } else if (request.ledger_index === 2812) {
      conn.send(
  ...
  mock.on('request_tx', function(request, conn) {
    ...
    }else if (request.transaction === hashes.SECOND_VALID_TRANSACTION_HASH)  {
      conn.send(createResponse(request, fixtures.tx.PaymentSecond))
  ...
  mock.on('request_account_lines', function(request, conn) {
    ...
    } else if (request.account === account.ACCOUNT) {

      conn.send(createResponse(request, fixtures.account_info.account_lines_test_account))
  ...
  mock.on('request_account_tx', function(request, conn) {
    ...
    if (request.account === account.ACCOUNT && request.forward === true && request.start !== undefined){
      conn.send(createResponse(request, fixtures.account_tx.test_account))
    } else if (request.account === account.ACCOUNT && request.forward === true){
      conn.send(createResponse(request, fixtures.account_tx.test_account_earliestfirst))
    } else if (request.account === account.ACCOUNT) {
      conn.send(createResponse(request, fixtures.account_tx.test_account))
    } else if (request.account === addresses.ACCOUNT) {
  ...
  mock.on('request_casinocoin_path_find', function(request, conn) {
    ...
    if (request.source_account === addresses.NOTFOUND) {
      ...
      if (request.destination_account ===
        'cn6GRVhmnkJ5WK2qeCAdzud8vsiy2onQMF') {
