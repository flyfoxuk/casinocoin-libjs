...
import fixtures from "./fixtures/casinocoind";
import addresses from "./fixtures/addresses";
import hashes from "./fixtures/hashes";
import transactionsResponse from "./fixtures/casinocoind/account-tx";
import accountLinesResponse from "./fixtures/casinocoind/account-lines";
import fullLedger from "./fixtures/casinocoind/ledger-full-38129.json";
...
function createResponse(request, response, overrides = {}) {
  ...
  const change = response.result && !_.isEmpty(overrides) ?
    { id: request.id, result } : { id: request.id };
  return JSON.stringify(_.assign({}, response, change));
}

function createLedgerResponse(request: any, response: any) {
...
const createMockCasinocoind = (port: any) => {
  const mock: WebSocketServer = new WebSocketServer({ port });
  _.assign(mock, EventEmitter2.prototype);

  const close = mock.close;
  mock.close = () => {
    if (mock.expectedRequests !== undefined) {
      const allRequestsMade = _.every(mock.expectedRequests, (counter: number) => {
        return counter === 0;
      });
      if (!allRequestsMade) {
  ...
  mock.expect = (expectedRequests: any) => {
    mock.expectedRequests = expectedRequests;
  };

  mock.on("connection", (conn: any) => {
    ...
    this.socket = conn;
    conn.config = {};
    conn.on("message", (requestJSON: any) => {
  ...
  mock.on("request_config", (request: any, conn: any) => {
    assert.strictEqual(request.command, "config");
    conn.config = _.assign(conn.config, request.data);
  });

  mock.on("request_test_command", (request: any, conn: any) => {
    ...
    if (request.data.disconnectIn) {
      ...
      getFreePort().then((newPort: number) => {
  ...
  mock.on("request_subscribe", (request: any, conn: any) => {
  ...
  mock.on("request_unsubscribe", (request: any, conn: any) => {
  ...
  mock.on("request_account_info", (request: any, conn: any) => {
  ...
  mock.on("request_ledger", (request: any, conn: any) => {
  ...
  mock.on("request_ledger_entry", (request: any, conn: any) => {
  ...
  mock.on("request_tx", (request: any, conn: any) => {
  ...
  mock.on("request_submit", (request: any, conn: any) => {
  ...
  mock.on("request_submit_multisigned", (request: any, conn: any) => {
    assert.strictEqual(request.command, "submit_multisigned");
    conn.send(createResponse(request, fixtures.submit.success));
  });

  mock.on("request_account_lines", (request: any, conn: any) => {
    if (request.account === addresses.ACCOUNT) {
  ...
  mock.on("request_account_tx", (request: any, conn: any) => {
    if (request.account === addresses.ACCOUNT) {
  ...
  mock.on("request_account_offers", (request: any, conn: any) => {
    if (request.account === addresses.ACCOUNT) {
  ...
  mock.on("request_book_offers", (request: any, conn: any) => {
  ...
  mock.on("request_casinocoin_path_find", (request: any, conn: any) => {
    let response = null;
  ...
  mock.on("request_gateway_balances", (request: any, conn: any) => {
...
export default createMockCasinocoind;
