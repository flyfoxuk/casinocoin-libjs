...
const RangeSet = CasinocoinAPI._PRIVATE.RangeSet;

describe("RangeSet", () => {
  it("addRange()/addValue()", () => {
  ...
  it("addValue()/addRange() -- malformed", () => {
    const r = new RangeSet();
    assert.throws(() => {
      r.addRange(2, 1);
    });
  ...
  it("parseAndAddRanges()", () => {
    const r = new RangeSet();
    r.parseAndAddRanges("4-5,7-10,1-2,3-3");
    assert.deepEqual(r.serialize(), "1-5,7-10");
  });
  it("parseAndAddRanges() -- single ledger", () => {
    const r = new RangeSet();
