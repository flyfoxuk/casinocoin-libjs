...
function makePositionalParams(params: any): any {
  return params.map((value: any) => value[_.keys(value)[0]]);
}

function makeNamedParams(params: any): any {
...
describe("http server integration tests", () => {
    ...
    it(testName, () => {
      return new Promise((resolve, reject) => {
        client.request(methodName, params, id,
          (err: any, result: any) => err ? reject(err) : resolve(testFunc(result)));
      });
    });
  }
  ...
  createTest(
    "getLedgerVersion",
    [],
    (result: any) => assert(_.isNumber(result.result)),
  );

  createTest(
    "getServerInfo",
    [],
    (result: any) => assert(_.isNumber(result.result.validatedLedger.ledgerVersion)),
  );
  ...
  createTest(
    ...
    (result: any) => assert.deepEqual(result.result, apiResponses.sign.normal),
  );
  ...
  createTest(
    ...
    (result: any) => assert.deepEqual(result.result, apiResponses.generateAddress),
  );
  ...
  createTest(
    "isConnected",
    [],
    (result: any) => assert(result.result),
  );
