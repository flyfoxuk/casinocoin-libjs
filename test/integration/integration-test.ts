...
function verifyTransaction(
  ...
  console.log("VERIFY...");
  return testcase.api.getTransaction(hash, options).then((data: any) => {
...
function testTransaction(
  ...
  return testcase.api.submit(signedData.signedTransaction)
    .then((data: any) => testcase.test.title.indexOf("multisign") !== -1 ?
      acceptLedger(testcase.api).then(() => data) : data).then((data: any) => {
...
function makeOrder(
  ...
  return api.prepareOrder(address, specification)
    ...
    .then((signed: any) => api.submit(signed.signedTransaction))
    .then(() => ledgerAccept(api));
}

function setupAccounts(testcase: any) {
  const api = testcase.api;
  ...
  const promise = payTo(api, "rMH4UxPrbuMa1spCBR98hLLyNJp4d8p4tM")
    ...
    .then(() => {
      return api.prepareSettings(masterAccount, { defaultCasinocoin: true })
        .then((data: any) => api.sign(data.txJSON, masterSecret))
        .then((signed: any) => api.submit(signed.signedTransaction))
        .then(() => ledgerAccept(api));
    })
...
describe("integration tests", () => {
  ...
  it("settings", () => {
    return this.api.getLedgerVersion().then((ledgerVersion: number) => {
      return this.api.prepareSettings(address,
        requests.prepareSettings.domain, instructions).then((prepared: any) =>
  ...
  it("trustline", () => {
    return this.api.getLedgerVersion().then((ledgerVersion: number) => {
      return this.api.prepareTrustline(address,
        requests.prepareTrustline.simple, instructions).then((prepared: any) =>
  ...
  it("payment", () => {
    ...
    return this.api.getLedgerVersion().then((ledgerVersion: number) => {
      return this.api.preparePayment(address,
        paymentSpecification, instructions).then((prepared: any) =>
    ...
    return this.api.getLedgerVersion().then((ledgerVersion: number) => {
      return this.api.prepareOrder(address, orderSpecification, instructions)
        ...
        .then((result: any) => {
          ...
          return this.api.getOrders(address).then((orders: any[]) => {
            assert(orders && orders.length > 0);
            const createdOrder = _.first(_.filter(orders, (order: any) => {
        ...
        .then((txData: any) => this.api.prepareOrderCancellation(
          address, { orderSequence: txData.Sequence }, instructions)
          .then((prepared: any) => testTransaction(this, "orderCancellation",
  ...
  it("getServerInfo", () => {
    return this.api.getServerInfo().then((data: any) => {
  ...
  it("getTransactions", () => {
    ...
    return this.api.getTransactions(address, options).then((transactionsData: any[]) => {
  ...
  it("getTrustlines", () => {
    ...
    return this.api.getTrustlines(address, options).then((data: any) => {
  ...
  it("getBalances", () => {
    const fixture = requests.prepareTrustline.simple;
    const options = _.pick(fixture, ["currency", "counterparty"]);
    return this.api.getBalances(address, options).then((data: any) => {
  ...
  it("getSettings", () => {
    return this.api.getSettings(address).then((data: any) => {
      assert(data);
  ...
  it("getOrderbook", () => {
    ...
    return this.api.getOrderbook(address, orderbook).then((book: any) => {
  ...
  it("getPaths", () => {
    ...
    return this.api.getPaths(pathfind).then((data: any) => {
  ...
  it("getPaths - send all", () => {
    ...
    return this.api.getPaths(pathfind).then((data: any) => {
      assert(data && data.length > 0);
      assert(_.every(data, (path: any) => {
        return parseFloat(path.source.amount.value) <=
          parseFloat(pathfind.source.amount.value);
      }));
      const path1 = data[0];
      assert(path1 && path1.source);
      assert.strictEqual(path1.source.address, pathfind.source.address);
      assert(path1.paths && path1.paths.length > 0);
    });
  });
...
describe("integration tests - standalone casinocoind", () => {
  const instructions = { maxLedgerVersionOffset: 10 };
  this.timeout(TIMEOUT);
  ...
  it("submit multisigned transaction", () => {
    const signers = {
      threshold: 2,
      weights: [
    ...
    let minLedgerVersion: any = null;
    return payTo(this.api, address).then(() => {
      return this.api.getLedgerVersion().then((ledgerVersion: any) => {
      ...
      return this.api.prepareSettings(
        ...
        .then((prepared: any) => {
          ...
          return this.api.submit(combined.signedTransaction)
            .then((response: any) => acceptLedger(this.api).then(() => response))
            .then((response: any) => {
