...
import computeTransactionHash from "casinocoin-libjs-hashes";
...
type GetTransactionsResponse = TransactionType[];

function parseBinaryTransaction(transaction: any) {
...
function parseAccountTxTransaction(tx: any) {
  const localizedTx = tx.tx_blob ? parseBinaryTransaction(tx) : tx;
  // casinocoind uses a different response format for "account_tx" than "tx"
  return parseTransaction(_.assign(
    {},
    localizedTx.tx,
    {
      meta: localizedTx.meta,
      validated: localizedTx.validated,
    },
  ));
}

function counterpartyFilter(filters: any, tx: TransactionType) {
...
function getAccountTx(
  ...
  return connection.request(request).then((response: any) =>
...
function getTransactions(
  ...
  if (options.start) {
    return getTransaction.call(this, options.start).then((tx: any) => {
